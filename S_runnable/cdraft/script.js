...
let COMPONENTS = new Map([
    ["Add a Text Variable", {
        c: "win", closeable: 1, modal: 1, id: "Add a Variable",
    ...
    ["Save JSON File", {
        c: "win", closeable: 1, modal: 1, id: "Download the JSON File?",
        v: ["Are you sure you want to download the JSON file?",
            { c: "btn", id: "btn_savejsfile", cap: "Yes" }]
    }],
    ["Load JSON File", {
        c: "win", closeable: 1, modal: 1, id: "Load the JSON File?",
        v: ["Select the JSON file you want to load?", { id: "myfile", cap: "", c: "file", accepts: "text" },
            { c: "btn", id: "btn_loadejsfile", cap: "Open" }]
    }],
    ["Generate Code", {
        c: "win", closeable: 1, modal: 1, id: "Generate Code",
        v: ["Generate Code", { c: "btn", id: "generateCode", cap: "Open" }]
    }],
    ["Run Code", {
        c: "win", closeable: 1, modal: 1, id: "Run Code?",
        v: ["Run Code?", { c: "btn", id: "runCode", cap: "Open" }]
    }]
    // link to index2.html
...
app.start(event => {
    app.display({
        cap: "JavaScript TouchDev (in-progress)", require: { c: ["grid", "opt"] }, style: "styles.css",
        value: [
            { id: "Your space" },
...
app.event({}, async event => {
    ...
    else if (event.u === "generateCode") {
        const code = await generateCode();
        console.log("------------GENERATE CODE--------\n", code);
    }

    else if (event.u === "runCode") {
    ...
    for (const element of intcode) {
        if (element.type == "LET_STATEMENT" && element.variableType === "STRING") {
            ...
            app.display({ U: "Your space", add: [{ c: "txt", v: "console.log(\"" + element.args + "\");" }] });
...
const runCode = async () => {
    const response = await fetch(`${SERVER_URL}/typescript/run`, {
        method: 'POST',
...
const generateCode = async () => {
    const response = await fetch(`${SERVER_URL}/typescript/code-generator`, {
        method: 'POST',
        body: JSON.stringify(intcode),
        headers: {
            "Content-Type": "application/json",
